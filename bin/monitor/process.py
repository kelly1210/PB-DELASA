...
def unfoundlib_out(unfound_lib_dir, samplename, unsplit_fa, unsplit_out, lib_out, constant1, constant2, UMI_Number):
    # unfound_lib_dir 
    # samplename 
    # unsplit_fa '.combined.unfound.lib.fa'
...
def compareList(list1,list2):
    new_list1 = list(filter(not_empty, list1))
...
# =============================================================================
    ...
    df00 = df0[df0['Dedup_count']>=float(cutoff)] 
    #print(df00.columns)
    
    if 'EF' in df00.columns:
        df00['EF'] = df00['EF'].astype(float)
        df00_temp = df00[df00['EF']>1] 
    ...
    if len(df) == 0:
       ...
       'TAG2_P_noline', 'TAG3_P_noline', 'Y', 'Y0']
        ...
        df.to_csv(outFileFilter, sep='\t', index=False) 
        dlpList=['0','0','0','0','0','0']
        return dlpList
    else:
    # =============================================================================
       ...
       # Y=1表示点线面阳性信号；Y0=1表示不存在线的面的阳性信号
        ...
        #print(df06)
        df06.to_csv(outFileFilter, sep='\t', index=False) 
        df06.to_excel(writer,'_count.filter', index=None) 
        writer.save()
        #writer.close()
    ...
    # =============================================================================
        ...
        dysythons_df_new.to_csv(dysythonFile, sep='\t', index=False)
...
def dot_judge(df00,dotNum,dedup_mean):
    # df00 dedup_counts>cutoff
    # dotNum top number
    tag123_lst = []
    dedup_mean = float(dedup_mean)
    df00.loc[(df00['Dedup_count'] >= 10 * dedup_mean), 'TAG_DOT'] = str(2)  
...
def line_judge(df00, tag12_sheet, CombineTAG1, CombineTAG2, TAG12_L):
    # df00 dedup_counts>cutoff
    # tag12_sheet cycle1_cycle2 BBlist
    # CombineTAG1 = 'TAG1'
    # CombineTAG2 = 'TAG2'
    # TAG12_L == 'TAG12_L'
    tag12_lst = []
...
def tags_excel(df, tags1,tags2,tags3):
    ...
    sheet1_new = sheet1_new.fillna('0') 
    preBB = ''
    ...
    tags1_top10 = list(sheet1_new.loc[:,tags1]) 
...
def funcNewSumSheet(oldsheet,sumsheet):
    if sumsheet.shape[0] >= 1:
        ...
        newsheet.drop(['index1','index2','index3','index4'],axis=1, inplace=True) 
        newsheet = newsheet.fillna('0') 

#!/home/pharmablock/anaconda3/bin python
#########################################################################
# Author: Keke Dong (dongkeke@pharmablock.com)
# Created Time: 2019-11-27
# Last Change: 1. changing directory structure from 'protein-concentration-sample-ALL files for librarys' to
# 'protein-library-ALL files for samples(sample name + '_' + library name + ...)'
...
def workflow():
    ...
    protein = protein_dict[prefix] # 此样本对应的蛋白质
    # library info
...
def eachlibraryDwar(batchName,config,run_steps,sample_dir,sample_info,library_info,protein,prefix,library_name,positive_str,positive_lst,roundNumber_dict,concentration_dict,merge_fa):
    ...
    cycles_dict = process.library_info(library_info)[2] #一个库对应一个cycles类型，3个或4个循环
    library_size_dict = process.library_info(library_info)[7]  # 一个库对应一个cycles类型，3个或4个循环
    # software or args
    ...
    code_smiles_dir = monitor_dir + '../../lib/Libraries/'
    #config["code_smiles_dir"] # config文件中定义好 code file 所在文件夹
    code_smiles = "{}/{}.code.smiles.txt".format(code_smiles_dir, library_name)
    ...
    if 'dwar' in run_steps:
        ...
        Combined_counts = process.file_line(merge_fa)/2 # fa 文件除以2得到reads数
        Split_counts = Combined_counts
        #Coding_counts = Combined_counts
        if not os.path.exists(sample_fullpath1):
            sample_fullpath1 = merge_fa
        Split_counts = process.file_line(sample_fullpath1)/2 # fa 文件除以2得到reads数
        ...
        if len(seq) == len(cycles):
            ...
            if cycleNM == 2:
                ...
                if cycle1_v[-2:] == cycle1[-2:] and cycle2_v[-2:] == cycle2[-2:]:
                    total_count3 += 1
                    key_seq = '\t'.join((cycle1_v, cycle2_v, header.split(':')[-1]))  # 添加UMI序列到整个匹配的序列
                ...
                if cycle1_v[-2:] == cycle1[-2:] and cycle2_v[-2:] == cycle2[-2:] and cycle3_v[-2:] == cycle3[-2:]:
                    total_count3 += 1
                    key_seq = '\t'.join((cycle1_v, cycle2_v, cycle3_v, header.split(':')[-1]))  # 添加UMI序列到整个匹配的序列
                    f_list_seq.write(key_seq + '\n')
            elif cycleNM == 4:
                cycle1_v = seq[0:len(cycle1)]
                cycle2_v = seq[len(cycle1):len(cycle1 + cycle2)]
                ...
                if cycle1_v[-2:] == cycle1[-2:] and cycle2_v[-2:] == cycle2[-2:] and cycle3_v[-2:] == cycle3[-2:] \
                    ...
                    total_count3 += 1
                    key_seq = '\t'.join((cycle1_v, cycle2_v, cycle3_v, cycle4_v, header.split(':')[-1]))  # 添加UMI序列到整个匹配的序列
                    f_list_seq.write(key_seq + '\n')
            elif cycleNM == 5:
                cycle1_v = seq[0:len(cycle1)]
                cycle2_v = seq[len(cycle1):len(cycle1 + cycle2)]
                ...
                if cycle1_v[-2:] == cycle1[-2:] and cycle2_v[-2:] == cycle2[-2:] and cycle3_v[-2:] == cycle3[-2:] \
                    ...
                    total_count3 += 1
                    key_seq = '\t'.join((cycle1_v, cycle2_v, cycle3_v, cycle4_v, cycle5_v, header.split(':')[-1]))  # 添加UMI序列到整个匹配的序列
                    f_list_seq.write(key_seq + '\n')
        else:
            log.write("%s\t%s\n" % (seq, len(seq)))
...
def CountDLP_app(CountFreqFile, SummaryCountFile,count_qc_lst,library_size):
    ...
    Compounds = df1.shape[0] #行数即富集的化合物个数

        ...
        if len(pair_name_lst) ==1 and pair_name_lst[0] == '0':
            ...
            if os.path.exists(casefile):
                ...
                if os.path.exists(ntcfile):
                    ...
                    #print(case_df['EF'+case_name2])
                    #case_df['EF'+case_name2] = case_df['EF'+case_name2].astype(float)
                    
                    #EF_min = case_df['EF'].min()
                    
                    #case_df['Dedup_count'+case_name2] = case_df['Dedup_count'+case_name2].astype(float)

                    # NTC
                    #ntc_df1['Dedup_count'+ntc_name2] = ntc_df1['Dedup_count'+ntc_name2].astype(float)
                    ...
                    #ntc_case_df['EF'] = case_df.apply(lambda row:enrichment_score(row['z-score'], row['NTC_z-score']), axis=1)
...
def txt2dwar_pair(ratio_file, dwar_file, dedup_filter_threshold,codesmilesfile,pair_name_lst,ntc_lst,sample):
    ...
    #print(count_f.columns)
    #count_f['EF'] = count_f['EF'].astype(float)
...
def main(casefile, ntcfile, ntc_name, ana_dir, proteinName_pair,sample, lib, pair_name_lst,temp_file, dwar_file,codesmilesfile):
	#case_dir = os.path.dirname(casefile)
...
if __name__ == "__main__":
    ...
    protein_dict = process.total_sample_info(input_SampleInfo)[0]  # 一个样本对应一个蛋白
    library_dict = process.total_sample_info(input_SampleInfo)[2]  # 一个样本文件对应多个库ID
    sample_lst_all = process.total_sample_info(input_SampleInfo)[8]  # 所有样本的样本名称
    ntc_dict = process.total_sample_info(input_SampleInfo)[10]  # 每个样本对应的ntc样本，该NTC样本有可能是0个，有可能是1个，有可能是多个
    pair_dict = process.total_sample_info(input_SampleInfo)[12]  # 每个样本对应的配对分析的样本，如果涉及到多个样本，则以逗号分隔，对这些样本的z-score，normalized z-score，EF进行合并并计算新的EF
    # (1)定义Pair列为0的值对应的样本，这样就先计算这些样本中的06.dwar中的文件，之后再进行合并
    sample_lst_0 = []
    pair_list = []
    print(pair_dict)
    ...
    print(sample_lst)
    # lib_lst = brc.library_info(input_LibInfo)[8]  # 所有库的名称
    library_size_dict = process.library_info(input_LibInfo)[7]
    # (2)case样本只有ntc对照的一个条件时，也保留该样本，并在该列表中去除已配对的另外的样本
    sample_lst_0_ntc=[]
    ntc_0_lst = []
    ...
    if len(sample_lst) != 0:
        # 只将需要做配对分析的样本输出到07.pair文件夹中，并输出dwar文件
        for sample in sample_lst:
            proteinName = protein_dict[sample]
            if 'NTC' in proteinName:
                print('NTC sample!!!')
            else:
                ntc_sample = ntc_dict[sample]
                ntc_name = '0'
                if ntc_sample != '0':
                ...
                # 如果没有配对样本，pair_name_lst=['0']
                pair_name_lst = list(set(pair_name.strip().split(',')))
                # qc. 检查样本（不加抑制剂）与其配对样本（加抑制剂）靶点是否相同，无论是否相同，都执行！
                ...
                for lib in lib_lst:
                    print(sample+'.'+lib)
                    #/home/software/PBDEL_pipeline_v3.0.1/bin/enumeration/LibDisorder
                    codesmilesfile = codesmiles_dir + '/' + lib + '.code.smiles.txt'
                    #ntcfile = '{}/{}/{}/{}.{}.count.freq.txt'.format(ana_dir,ntc_name,lib,ntc_sample,lib)
                    #casefile = '{}/{}/{}/{}.{}.count.freq.txt'.format(ana_dir,proteinName,lib,sample,lib)
                    ...
                    #print(casefile)
                    #print(ntcfile)
                    #sample_fullpath = ana_dir + "/" + protein_dict[sample] + '/' + lib + '/' + sample + '.' + lib + '.count.freq.txt'
                    if os.path.exists(casefile_filter) and os.path.exists(ntcfile_filter):
                        #p.apply_async(main,args=(casefile_filter, ntcfile_filter, ana_dir, proteinName_pair,sample, lib, pair_name_lst, temp_file,dwar_file,codesmilesfile))

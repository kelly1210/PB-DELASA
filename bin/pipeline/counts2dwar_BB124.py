...
def dedup_filter(ratio_file):
    ...
    BB1 = set(test['TAG1'].tolist())
    BB2 = set(test['TAG2'].tolist())
...
<filter4="#double#	TAG2">
<filter5="#string#	TAG2_seq">
<filter6="#string#	TAG2_structure">
<filter7="#double#	TAG4">
...
<filter19="#string#	TAG1_DLP">
<filter20="#string#	TAG2_DLP">
...
def DotLinePlane(ratio_file, ratio_file_filter):
    ...
    BB1 = set(count_f_filter['TAG1'].tolist())
    BB2 = set(count_f_filter['TAG2'].tolist())
    ...
    # print(BB3)
    for Tag1 in BB1:
        count_f_filter_tag1 = count_f_filter[count_f_filter['TAG1'] == Tag1]
        BB1_f1 = count_f_filter_tag1.shape[0] # 同一个BB1的行数，即
        BB2_f1 = set(count_f_filter_tag1['TAG2'].tolist())
        BB3_f1 = set(count_f_filter_tag1['TAG4'].tolist())
        if (BB1_f1 == 1):
            count_f_filter.loc[(count_f_filter['TAG1'] == Tag1) & (count_f_filter['Dedup_count'] >= 100*dedup_mean), 'TAG1_DLP'] = str(1.11) #判断清晰的点
            count_f_filter.loc[(count_f_filter['TAG1'] == Tag1) & (count_f_filter['Dedup_count'] < 100*dedup_mean),'TAG1_DLP'] = str(1.10) # 判断点，1.10表示tag1，点，清晰程度是0
        elif (len(BB2_f1) - BB1_f1 == 0) and (len(BB3_f1) - BB1_f1 == 0):
            # 任意两点都不在一条线上就认为不成面（即分散的点），如果2个点以上才算
            count_f_filter.loc[count_f_filter['TAG1'] == Tag1, 'TAG1_DLP'] = str(0)
        else:
            if len(BB3_f1) >= (0.9 * BB3_len) and len(BB2_f1) >= (0.9 * BB2_len):
                count_f_filter.loc[count_f_filter['TAG1'] == Tag1,'TAG1_DLP'] = str(1.34)
                # 判断面
            elif (len(BB3_f1) >= (0.9 * BB3_len) and len(BB2_f1) >= (0.8 * BB2_len)) or (len(BB3_f1) >= (0.8 * BB3_len) and len(BB2_f1) >= (0.9 * BB2_len)):
                count_f_filter.loc[count_f_filter['TAG1'] == Tag1,'TAG1_DLP'] = str(1.33)
            elif len(BB3_f1) >= (0.8 * BB3_len) and len(BB2_f1) >= (0.8 * BB2_len):
                count_f_filter.loc[count_f_filter['TAG1'] == Tag1,'TAG1_DLP'] = str(1.32)
            elif len(BB2_f1) <= (0.2 * BB2_len) and len(BB3_f1) >= (0.8 * BB3_len):
                for Tag2 in BB2_f1:
                    BB3_f2 = count_f_filter_tag1[count_f_filter_tag1['TAG2']==Tag2].shape[0]
                    # 当BB1和BB2固定时，判断BB3的个数
                    if BB3_f2 >= (0.8 * BB3_len):
                        count_f_filter.loc[(count_f_filter['TAG1'] == Tag1) & (count_f_filter['TAG2'] == Tag2),'TAG1_DLP'] = str(1.24)
                        # 判断线，很明显的线.1.21：1表示tag1,2表示线，1表示线明显程度。明显程度逐渐递增（0,1,2）
                    elif BB3_f2 >= (0.8 * len(BB3_f1)):
                        count_f_filter.loc[(count_f_filter['TAG1'] == Tag1) & (count_f_filter['TAG2'] == Tag2),'TAG1_DLP'] = str(1.23)
                        # 判断线，相对没那么明显的线
                ...
                for Tag3 in BB3_f1:
                    ...
                    if BB2_f2 >= (0.8 * BB2_len):
                        count_f_filter.loc[(count_f_filter['TAG1'] == Tag1) & (count_f_filter['TAG4'] == Tag3),'TAG1_DLP'] = str(1.24)
                        # 判断线，很明显的线.1.21：1表示tag1,2表示线，1表示线明显程度。明显程度逐渐递增（0,1,2）
                    elif BB2_f2 >= (0.8 * len(BB2_f1)):
                        count_f_filter.loc[(count_f_filter['TAG1'] == Tag1) & (count_f_filter['TAG4'] == Tag3),'TAG1_DLP'] = str(1.23)
                        # 判断线，相对没那么明显的线
            elif (len(BB2_f1) >= max(3, (0.5 * BB2_len))) or (len(BB3_f1) >= max(3, (0.5 * BB3_len))):
                count_f_filter.loc[(count_f_filter['TAG1'] == Tag1), 'TAG1_DLP'] = str(1.22)

            # 如果面和线都不明显，一条线上只有3个点或以上时，DLP对应的值都为零时，缩小条件重新判定点线面
            elif BB1_f1 >= 3:
                # 判断不清晰的面
                if len(BB2_f1) >= max(3, 0.2*BB1_f1) and len(BB3_f1) >= max(3, 0.2*BB1_f1):
                    count_f_filter.loc[count_f_filter['TAG1'] == Tag1, 'TAG1_DLP'] = str(1.31)
                elif len(BB2_f1) >= max(3, 0.2*BB1_f1) or len(BB3_f1) >= max(3, 0.2*BB1_f1):
                    count_f_filter.loc[count_f_filter['TAG1'] == Tag1, 'TAG1_DLP'] = str(1.30)
                elif len(BB2_f1) < max(3, 0.2*BB1_f1):
                    for Tag2 in BB2_f1:
                        BB3_f3 = count_f_filter_tag1[count_f_filter_tag1['TAG2']==Tag2].shape[0]
                        if BB3_f3 >= 3:
                            count_f_filter.loc[(count_f_filter['TAG1'] == Tag1) & (count_f_filter['TAG2'] == Tag2), 'TAG1_DLP'] = str(1.21)
                        else:
                            count_f_filter.loc[(count_f_filter['TAG1'] == Tag1) & (count_f_filter['TAG2'] == Tag2), 'TAG1_DLP'] = str(1.20)
                elif len(BB3_f1) < max(3, 0.2*BB1_f1):
                    for Tag3 in BB2_f1:
                        BB2_f3 = count_f_filter_tag1[count_f_filter_tag1['TAG4']==Tag3].shape[0]
                        if BB2_f3 >= 3:
                            count_f_filter.loc[(count_f_filter['TAG1'] == Tag1) & (count_f_filter['TAG4'] == Tag3), 'TAG1_DLP'] = str(1.21)
                        else:
                            count_f_filter.loc[(count_f_filter['TAG1'] == Tag1) & (count_f_filter['TAG4'] == Tag3), 'TAG1_DLP'] = str(1.20)

######################################################################################################################


    for Tag2 in BB2:
        count_f_filter_tag2 = count_f_filter[count_f_filter['TAG2'] == Tag2]
        BB2_f1 = count_f_filter_tag2.shape[0] # 同一个BB2的行数，即
        BB1_f1 = set(count_f_filter_tag2['TAG1'].tolist())
        BB3_f1 = set(count_f_filter_tag2['TAG4'].tolist())
        if BB2_f1 == 1:
            count_f_filter.loc[(count_f_filter['TAG2'] == Tag2) & (count_f_filter['Dedup_count'] >= 100 * dedup_mean), 'TAG2_DLP'] = str(2.11)
            count_f_filter.loc[(count_f_filter['TAG2'] == Tag2) & (count_f_filter['Dedup_count'] < 100 * dedup_mean),'TAG2_DLP'] = str(2.10) # 判断点，2.10表示tag2，点，清晰程度是0
        elif len(BB1_f1) == BB2_f1 and len(BB3_f1) == BB2_f1:
            # 任意两点都不在一条线上就认为不成面
            count_f_filter.loc[count_f_filter['TAG2'] == Tag2, 'TAG2_DLP'] = str(0)
        else:
            if len(BB3_f1) >= (0.9 * BB3_len) and len(BB1_f1) >= (0.9 * BB1_len):
                count_f_filter.loc[count_f_filter['TAG2'] == Tag2,'TAG2_DLP'] = str(2.34)
                # 判断面
            elif (len(BB3_f1) >= (0.9 * BB3_len) and len(BB1_f1) >= (0.8 * BB1_len)) or (len(BB3_f1) >= (0.8 * BB3_len) and len(BB1_f1) >= (0.9 * BB1_len)):
                count_f_filter.loc[count_f_filter['TAG2'] == Tag2,'TAG2_DLP'] = str(2.33)
            elif len(BB3_f1) >= (0.8 * BB3_len) and len(BB1_f1) >= (0.8 * BB1_len):
                count_f_filter.loc[count_f_filter['TAG2'] == Tag2,'TAG2_DLP'] = str(2.32)
            elif len(BB1_f1) <= (0.2 * BB1_len) and len(BB3_f1) >= (0.8 * BB3_len):
                for Tag1 in BB1_f1:
                    BB3_f2 = count_f_filter_tag2[count_f_filter_tag2['TAG1']==Tag1].shape[0]
                    # 当BB1和BB2固定时，判断BB3的个数
                    if BB3_f2 >= (0.8 * BB3_len):
                        count_f_filter.loc[(count_f_filter['TAG2'] == Tag2) & (count_f_filter['TAG1'] == Tag1),'TAG2_DLP'] = str(2.24)
                        # 判断线，很明显的线.2.21：2表示tag2,2表示线，1表示线明显程度。明显程度逐渐递增（0,1,2）
                    elif BB3_f2 >= (0.8 * len(BB3_f1)):
                        count_f_filter.loc[(count_f_filter['TAG2'] == Tag2) & (count_f_filter['TAG1'] == Tag1),'TAG2_DLP'] = str(2.23)
                        # 判断线，相对没那么明显的线
                ...
                for Tag3 in BB3_f1:
                    ...
                    if BB1_f2 >= (0.8 * BB1_len):
                        count_f_filter.loc[(count_f_filter['TAG2'] == Tag2) & (count_f_filter['TAG4'] == Tag3),'TAG2_DLP'] = str(2.24)
                        # 判断线，很明显的线.1.21：1表示tag1,2表示线，1表示线明显程度。明显程度逐渐递增（0,1,2）
                    elif BB1_f2 >= (0.8 * len(BB1_f1)):
                        count_f_filter.loc[(count_f_filter['TAG2'] == Tag2) & (count_f_filter['TAG4'] == Tag3),'TAG2_DLP'] = str(2.23)
                        # 判断线，相对没那么明显的线
            elif (len(BB1_f1) >= max(3, (0.5 * BB1_len))) or (len(BB3_f1) >= max(3, (0.5 * BB3_len))):
                count_f_filter.loc[(count_f_filter['TAG2'] == Tag2), 'TAG2_DLP'] = str(2.22)

            # 如果面和线都不明显，一条线上只有3个点或以上时，DLP对应的值都为零时，缩小条件重新判定点线面
            elif BB2_f1 >= 3:
                # 判断不清晰的面
                if len(BB1_f1) >= max(3, 0.2 * BB2_f1) and len(BB3_f1) >= max(3, 0.2 * BB2_f1):
                    count_f_filter.loc[count_f_filter['TAG2'] == Tag2, 'TAG2_DLP'] = str(2.31)
                elif len(BB1_f1) >= max(3, 0.2 * BB2_f1) or len(BB3_f1) >= max(3, 0.2 * BB2_f1):
                    count_f_filter.loc[count_f_filter['TAG2'] == Tag2, 'TAG2_DLP'] = str(2.30)
                elif len(BB1_f1) < max(3, 0.2 * BB2_f1):
                    for Tag1 in BB1_f1:
                        BB3_f3 = count_f_filter_tag2[count_f_filter_tag2['TAG1'] == Tag1].shape[0]
                        if BB3_f3 >= 3:
                            count_f_filter.loc[(count_f_filter['TAG2'] == Tag2) & (
                                        count_f_filter['TAG1'] == Tag1), 'TAG2_DLP'] = str(2.21)
                        else:
                            count_f_filter.loc[(count_f_filter['TAG2'] == Tag2) & (
                                        count_f_filter['TAG1'] == Tag1), 'TAG2_DLP'] = str(2.20)
                elif len(BB3_f1) < max(3, 0.2 * BB2_f1):
                    for Tag3 in BB1_f1:
                        BB1_f3 = count_f_filter_tag2[count_f_filter_tag2['TAG4'] == Tag3].shape[0]
                        if BB1_f3 >= 3:
                            count_f_filter.loc[(count_f_filter['TAG2'] == Tag2) & (
                                        count_f_filter['TAG4'] == Tag3), 'TAG2_DLP'] = str(2.21)
                        else:
                            count_f_filter.loc[(count_f_filter['TAG2'] == Tag2) & (
                                        count_f_filter['TAG4'] == Tag3), 'TAG2_DLP'] = str(2.20)
    ...
    for Tag3 in BB3:
        ...
        BB2_f1 = set(count_f_filter_tag3['TAG2'].tolist())
        BB1_f1 = set(count_f_filter_tag3['TAG1'].tolist())
        if BB3_f1 == 1:
            ...
            if len(BB1_f1) >= (0.9 * BB1_len) and len(BB2_f1) >= (0.9 * BB2_len):
                ...
                for Tag2 in BB2_f1:
                    BB1_f2 = count_f_filter_tag3[count_f_filter_tag3['TAG2']==Tag2].shape[0]
                    # 当BB1和BB2固定时，判断BB3的个数
                    if BB1_f2 >= (0.8 * BB1_len):
                        count_f_filter.loc[(count_f_filter['TAG4'] == Tag3) & (count_f_filter['TAG2'] == Tag2),'TAG4_DLP'] = str(4.21)
                        # 判断线，很明显的线.1.21：1表示tag1,2表示线，1表示线明显程度。明显程度逐渐递增（0,1,2）
                    elif BB1_f2 >= (0.8 * len(BB1_f1)):
                        count_f_filter.loc[(count_f_filter['TAG4'] == Tag3) & (count_f_filter['TAG2'] == Tag2),'TAG4_DLP'] = str(4.20)
                        # 判断线，相对没那么明显的线
            elif len(BB1_f1) <= (0.2 * BB1_len) and len(BB2_f1) >= (0.8 * BB2_len):
                for Tag1 in BB1_f1:
                    BB2_f2 = count_f_filter_tag3[count_f_filter_tag3['TAG1']==Tag1].shape[0]
                    # 当BB1和BB3固定时，判断BB2的个数
                    if BB2_f2 >= (0.8 * BB2_len):
                        count_f_filter.loc[(count_f_filter['TAG4'] == Tag3) & (count_f_filter['TAG1'] == Tag1),'TAG4_DLP'] = str(4.21)
                        # 判断线，很明显的线.1.21：1表示tag1,2表示线，1表示线明显程度。明显程度逐渐递增（0,1,2）
                    elif BB2_f2 >= (0.8 * len(BB2_f1)):
                        count_f_filter.loc[(count_f_filter['TAG4'] == Tag3) & (count_f_filter['TAG1'] == Tag1),'TAG4_DLP'] = str(4.20)
                        # 判断线，相对没那么明显的线
            ...
            # 如果面和线都不明显，一条线上只有3个点或以上时，DLP对应的值都为零时，缩小条件重新判定点线面
                ...
                if len(BB1_f1) >= max(3, 0.2 * BB3_f1) and len(BB2_f1) >= max(3, 0.2 * BB3_f1):
                    ...
                    for Tag1 in BB1_f1:
                        BB2_f3 = count_f_filter_tag3[count_f_filter_tag3['TAG1'] == Tag1].shape[0]
                        if BB2_f3 >= 3:
                            count_f_filter.loc[(count_f_filter['TAG4'] == Tag3) & (
                                        count_f_filter['TAG1'] == Tag1), 'TAG4_DLP'] = str(4.21)
                        else:
                            count_f_filter.loc[(count_f_filter['TAG4'] == Tag3) & (
                                        count_f_filter['TAG1'] == Tag1), 'TAG4_DLP'] = str(4.20)
                elif len(BB2_f1) < max(3, 0.2 * BB3_f1):
                    for Tag2 in BB1_f1:
                        BB1_f3 = count_f_filter_tag3[count_f_filter_tag3['TAG2'] == Tag2].shape[0]
                        if BB1_f3 >= 3:
                            count_f_filter.loc[(count_f_filter['TAG4'] == Tag3) & (
                                        count_f_filter['TAG2'] == Tag2), 'TAG4_DLP'] = str(4.21)
                        else:
                            count_f_filter.loc[(count_f_filter['TAG4'] == Tag3) & (
                                        count_f_filter['TAG2'] == Tag2), 'TAG4_DLP'] = str(4.20)

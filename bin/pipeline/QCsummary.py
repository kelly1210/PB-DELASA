# -*- coding: utf-8 -*-
"""
Created on Tue Nov 24 11:07:38 2020

@author: dong_keke

Contact: dong_keke@PharmaBlock.com

Copyright: "Copyright 2020, PharmaBlock"
...
def modified_qc_summary(total_info_file, QC_batch, QC_sample, contaminationFile, QC_lib, sumQC):
    ...
    BatchName = QC_batch_f['BatchName'][0] # 得到批次数据的名称
    RawTotalReads = QC_batch_f['raw_single_reads(R1/R2)'][0] # 得到批次数据的原始reads数（R1/R2）
    TrimTotalReads = QC_batch_f['trim_single_reads(R1/R2)'][0] # 得到批次数据的trim后（Q30）的reads数（R1/R2）

    sample_lst = process.total_sample_info(total_info_file)[8] # 所有样本的样本名称
    qpcr_dict = process.total_sample_info(total_info_file)[11] # 所有样本的样本名称
    ...
    for sample in sample_lst:
        if sample not in qc_sample_lst:
            ...
            if qpcr!=0:
                ...
                qpcr_ratio = '0'
            # Dedup_ratio = '%.4f'%(Dedup_counts/Raw_counts)
            # 每个样本行
            # 输出样本对应的数目最大的序列及其序列占比
            #seq_Raw_reads = QC_sample_contamination['RawReads'][0]
            seq_counts_max = QC_sample_contamination[sample].max()
            seq_counts_ratio = 0
            if Trim_single_reads != 0:
                seq_counts_ratio = '%.4f'%(seq_counts_max / int(Trim_single_reads)) # 输出百分数
            #print(QC_sample_contamination[QC_sample_contamination[sample] == seq_counts_max]['ContaminSeq'].tolist()[0])
            ...
            #print(seq_counts_ratio)
            #print(type(seq_counts_ratio))
...
def DEL_QC_stat(batchname,sum_qc,sam_sum_qc,m_sum_qc,sinfo,libinfo):
    ...
    #sam_sum_df1.columns = ["SampleID","Raw_Q30","Trim_Q30","LibraryID"]
    # LowQuality_rate统计
